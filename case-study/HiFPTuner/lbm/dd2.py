...
def getPrediction(data):
  ...
  with torch.no_grad():
    ...
    time_preds = (time_predictions > 0.49).float() # finetune : 0.49 scratch: 0.5498
    error_predictions = error_predictor(data, DEVICE)
    error_preds = (error_predictions > 0.4).float() # finetune : 0.4 scratch: 0.544
...
def not_run_orig_config(search_config, original_config, benchmark):
  ...
  with open('./time.txt', "w") as f:
    ...
    # f.write(str(165.714959306))

  config_info_dict["runtime"] = avg_runtime
  # config_info_dict["runtime"] = 165.714959306
