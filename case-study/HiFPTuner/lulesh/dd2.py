...
map_json = json.loads(open(sys.argv[7], 'r').read())
real_conf = {}
...
time_predictor.load_state_dict(torch.load("../src/time_finetune.pt"))
print("Runtime time_predictor {} is loaded.".format("../src/time_finetune.pt"))
time_predictor.to(DEVICE)
time_predictor.eval()

error_predictor = model.HeteroGNN(GRAPHS.split('_'))
error_predictor.load_state_dict(torch.load("../src/error_scratch.pt"))
print("Error error_predictor {} is loaded.".format("../src/error_scratch.pt"))
...
def getPrediction(data):
  ...
  with torch.no_grad():
    ...
    time_preds = (time_predictions > 0.5).float() 
    error_predictions = error_predictor(data, DEVICE)
    error_preds = (error_predictions > 0.5).float() 
...
def run_config(search_config, original_config, benchmark):
  ...
  # print(f"Runtime prediction: {time_predictions} ==> {time_preds}")
  # logging.info(f"Runtime prediction: {time_predictions} ==> {time_preds}")
  # print(f"Error prediction: {error_predictions} ==> {error_preds}")
  # logging.info(f"Error prediction: {error_predictions} ==> {error_preds}")
...
def dd_search_config(change_set, type_set, switch_set, real_conf, original_config, benchmark, div, original_score):
  #
  ...
  for i in range(0, len(delta_change_set)):
    ...
    if len(delta_change) > 0:
      ...
      # map_json, original_conf, change_set ==> real_conf
      real_conf = utilities.get_real_conf(map_json, real_conf, change_set)

      # apply change for variables in delta
      utilities.to_highest_precision(delta_change, delta_type, switch_set)
      # map_json, original_conf, change_set ==> real_conf
      real_conf = utilities.get_real_conf(map_json, real_conf, delta_change)

      # record i if config passes
      if run_config(real_conf, original_config, benchmark) == 1 and utilities.get_dynamic_score() < original_score:
    ...
    if len(delta_inv_change) > 0 and div > 2:
      ...
      # map_json, original_conf, change_set ==> real_conf
      real_conf = utilities.get_real_conf(map_json, real_conf, delta_inv_change)
  ...
  if pass_inx != -1:
    ...
    if len(pass_change_set) > 1:
      ...
      # map_json, original_conf, change_set ==> real_conf
      real_conf = utilities.get_real_conf(map_json, real_conf, change_set)
      dd_search_config(pass_change_set, pass_type_set, pass_switch_set, real_conf, original_config, benchmark, 2, original_score)
    else:
      utilities.to_2nd_highest_precision(change_set, type_set, switch_set)
      # map_json, original_conf, change_set ==> real_conf
      real_conf = utilities.get_real_conf(map_json, real_conf, change_set)
      ...
      # map_json, original_conf, change_set ==> real_conf
      real_conf = utilities.get_real_conf(map_json, real_conf, pass_change_set)
    return
  ...
  if div >= len(change_set):
    ...
    dd_search_config(change_set, type_set, switch_set, real_conf, original_config, benchmark, 2*div, original_score)
    return


def search_config(change_set, type_set, switch_set, real_conf, original_config, benchmark, original_score):
  # search from bottom up
  utilities.to_2nd_highest_precision(change_set, type_set, switch_set)
  # map_json, original_conf, change_set ==> real_conf
  real_conf = utilities.get_real_conf(map_json, real_conf, change_set)

  if run_config(real_conf, original_config, benchmark) != 1 or utilities.get_dynamic_score() > original_score:
    dd_search_config(change_set, type_set, switch_set, real_conf, original_config, benchmark, 2, original_score)
...
def main():
  ...
  for level in [1, 0]:
    ...
    #
    search_conf_file = f"search_config_l{level}.json"
    ...
    if prune_flag:
      for idx in search_conf:
        for var in search_conf[idx]["community"]:
          if var in map_json:
            key = map_json[var]
            if 'float' in glob_search_conf[key]['type']:
              search_conf[idx]["type"] = ["float"]
              break
      tmp = json.dumps(search_conf, indent=4)
      with open(f"updated_{search_conf_file}", 'w') as f:
        f.write(tmp)
    prune_flag = 1
    ...
    utilities.to_highest_precision(change_set, type_set, switch_set)
    # map_json, original_conf, change_set ==> real_conf
    real_conf = utilities.get_real_conf(map_json, original_conf, change_set)

    if level == 1:
      not_run_orig_config(real_conf, original_conf, benchmark)
      global original_runtime
    ...
    while not utilities.is_empty(type_set):
      search_config(change_set, type_set, switch_set, real_conf, original_conf, benchmark, original_score)
